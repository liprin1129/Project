...
There are a large amount of data set collected by 854,064 players from the on-line game called Axon. Using the on-line game data set, how individuals learn in practice is identifyed by fitting possible underlying learning models to the data set. The power law model is fitted to the data set, as well as other several functions; exponential, polynomial, step, and piecewise functions combined with various shape of functions. This analysis showed that many individuals follow the power law rather than other practice law in on-line game practice. Additionally, the trade-off between exploitation and exploration in reinforcement learning as the skill acquisition process in practice is discussed, based on the result of the curve fitting.
...
Firstly, thank Dr. Stafford for the kind supervision. He has supervised thoroughly how to appropriately deal with the unexpected problems in this data analysis, and encouraged me a lot when I faced many difficulties. Also, thank Dr. Vasilaki who is a lecturer in Computer Science at University of Sheffield, because the artificial intelligence algorithm, this analysis uses for the clustering task, was discussed though the Adaptive Intelligence model.
Finally, the large amount of game data set is given by PRELOAD for the Wellcome Collection \cite{stuart2009Axon}. 
...
Shape of learning curve may reflect different underlying learning processes. For example, in insight learning, the learning curve may follow slightly fluctuated level and sudden dramatic increase at some points. If this feature between performances and practises is trained to identify the underlying learning curve, it would result in a step-like model. Another example is a power curve. It is well known "law of practice" in Psychology \cite{newell1981mechanisms}. Many different skill acquisition tasks can be described well under the power law \cite{lacroix2006introduction}. Howard (2014) said that power law is "correct and general" to explain learning processes. If subjectives show a learning performance of power law, their learning performance will be enhanced progressively early in learning, but will become slower and slower, and then will reach to asymptote. Therefore, "curve fitting with benefit of a model" would provide effective understanding of skill acquisition and learning processes \cite{newell1981mechanisms}.

\bigskip
Power function has been used as a fundamental and popular practice law in Psychology, such as Ericsson et al. (1993) proved the expert performance theory that "performance increases monotonically" following with the power law, although "the form of the practice law may also seem to be an unsolvable technical issue, rather than an important psychological question" \cite{heathcote2000power}.

\bigskip
Individuals may show different shapes of learning process beyond power function. %which has monotonic shape. 
Some may follow exponential law, and others may have "multi-step changes in behaviour" if experiences and insight may reinforce each other and can be associated to one another \cite{gallistel2004learning}. There are several possible functions which might fit to the curve of skill acquisition, and may describe different learning processes; step function, power law, piecewise power function, and so on. Because the power law is often said that it is the outcome of averaging these possible functions, individuals can be categorised into several groups in which they show similar learning patterns on these possible functions.

\begin{comment}
Moreover, by inspecting people in those groups, 'how they practice' to get the learning curve and 'which features' having an affect upon individuals can be found. Then suggestions for better learning for individuals may be categorised and proposed.
\end{comment}

\bigskip
In contrary with the traditional approaches for investigation of learning curve which usually brought experts into a lab to test skill acquisition, this analysis uses a large amount of data set collected from on-line game. The traditional approaches lack "comprehensive details" of subjectives' training and the exact feature of "the history of their practice which is related to the skilled performance" \cite{stafford2014tracing}. Also, it has a limited number of subjectives. It might thus result in the power curve for the investigation of skill acquisition.

However, the computer game provides an abundant "record of every action in the history" of practice, as well as a large number of samples of players \cite{stafford2014tracing}. This large number of data set and records may be able to allow identifying the underlying practice law which are hidden under the power law of averaged learning curves.
...
It has been often argued that the power law is an "artifact of averaging across individual learning curves" \cite{donner2015piecewise}. Power law of skill acquisition has a single monotonic power curve to predict individual performance. However, learning processes might be dynamic, and show various shape of curves. Individual may show a "step-like performance" \cite{gallistel2004learning} represented as "aha! moment" \cite{kounios2009aha}, which is involved in the abrupt increases in practice. 

\bigskip
Even among monotonic shape of curves, whether the power law can be representative of learning curve or not is controversial. Heathcote et al. (2000) strongly insisted that the power law should be repealed, and that the exponential function with three parameters can describe the not-averaged data set better than the power functions. 

Gaschler et al. (2007) tested the exponential function with various parameters to the data sets of chess game practice, and they found that "the exponential function with three parameters was better than the power function in fitting" \cite{gaschler2007playing}. 

\bigskip
Individuals may follow the learning process showing the shape of exponential curve rather than the power law. If individuals in the exponential law are averaged together, a grouped learning curve will be the shape of power law \cite{murre2011power}. In addition, if all other possible shapes of learning curves including the exponential curves are squashed into one learning curve, the power law has a possibility to be yielded as the outcome from the averaging those learning curves.

\bigskip
Moreover, as explained above, the traditional way to investigate the practice laws in the laboratories may restrict the amount of data, as well as the history of practice for the analysis \cite{stafford2014tracing}. The problem of this lack of data may cause different results rather than the true information.

\bigskip
To sum up, problems in current practice laws can be summarised as below,

\begin{itemize}
\item not being able to categorise the various learning processes,
\item averaging possible individual learning curves,
\item not enough information for the identification of skill acquisition,
\end{itemize}
and those problems are dealt with, and discussed in this analysis.
...
In order to solve these three main issues which is explained above, this analysis exploits the on-line game data set called Axon. In which, game play data of players are recorded in accordance with their attempts. Although the collected data set has a lot of information including the history of their practice, it also includes wrong information, such as the attempt time is recorded wrongly and the second attempt time is later than the first trial time. Therefore, this wrong information has to be filtered before the analysis is implemented.  

At the same time, it is assumed that individuals may show different learning patterns. Therefore, players will be categorised into the several groups in which they show the similar learning processes. 

\bigskip
Because the on-line data set has a large amount of data, the performance of these processes will be very slow, and the it may take a lot of time to analyse such the large amount of data set. In order to solve it, several methods will be introduced, such as using multiple cores of CPU.

\bigskip
To understand which are the underlying learning processes under the averaged power law, two theories will be tested that the power law model is the averaged curve according to individual exponential curves \cite{murre2011power, gaschler2007playing}, and that many tasks show step-like performance \cite{gallistel2004learning}. Therefore, function fitting for the possible underlying models; polynomial, exponential, power, stet, and piecewise functions with various combinations, to the data set will be implemented.

Through these tasks, it can be identified which underlying learning curves disappear under the averaged power law model. 

\bigskip
Finally, the trade-off in skill acquisition in practice, reinforcement learning, will be able to discussed after identifying the individual learning curves.
...
Two gantt charts are attached in appendix. The first one is the gantt chart for the project time management, and the second chart is the actual time management.
...
The reason for second step is that plays more than 300 attempts did not show many differences after 300 attempts, and they were removed. Next, players having unrecognisable time information were also discarded. Finally, game play data for individuals were recorded discontinuously. In other words, there are non-recorded game play data between the first attempt and the last attempt for each players. The reason why this problem happened is unknown, though it is obvious that individuals with this issue have to be filtered appropriately.

As a result, raw data of 854,064 players with 4, 038,802 plays were cleaned, and then reduced to 22,832 individuals.
...
Because the raw data set is a large amount, the process time for data curation would take long time. Commonly, using multithreading is the easiest way to solve this operating time problem. However, unfortunately Python does not allow multiple statements at a kernel \cite{python2016multiprocessing}. Therefore, the multiprocessing method which uses multiple cores in CPU is used.

\bigskip
In using a single core of Intel xeon processor, E5-2698V3, the data curation task took almost about 3 days. However, using multiprocessing under the 32 threads of 16 multiple cores with 2.3 GHz Processor Base Frequency showed the high performance; it only took below 3 hours.
%%%%%% comment %%%%%%%%%%%%%
\begin{comment}
(Python 2001)
...
\begin{figure}[htpb]
    ...
    \framebox{\parbox[t][0.7\textwidth][t]{0.95\textwidth}{
        ...
        \end{center} }}
	\caption{Competitive neuron diagram. When an input enters the network, it generates output values according to weights between an input and output neurons (blue arrows). Output neurons compete with each other (red arrows), then only one output fires on an input pattern. Output neurons are considered only having binary $0/1$ output.}
...
\begin{equation} \label{eq:competitive2}
    \vec{\mathbf{w}}_k^\top\vec{\mathbf{x}}^\mu > \vec{\mathbf{w}}_i^\top\vec{\mathbf{x}}^\mu \quad \textrm{for all i},
\end{equation}
where winner neuron is $\vec{\mathbf{w}}_k^\top$, and $\vec{\mathbf{x}}^\mu$ is an input of pattern $\mu$.
...
\begin{equation} \label{eq:competitive3}
    \quad |\vec{x}^\mu - \vec{w}_k^\top| \leq |\vec{x}^\mu - \vec{w}_i^\top|, \quad
...
%output neurons have a value of $\vec{w}^\top\vec{x}^\mu$, thus the largest output is always win. 
...
\begin{equation}\label{eq:inner_product}
    \vec{\mathbf{w}}^\top \cdot \vec{\mathbf{x}}^\mu = \vec{\mathbf{w}}^\top\vec{\mathbf{x}}^\mu \cos(\theta),
\end{equation}
...
\begin{equation}\label{eq:distance}
    \begin{split}
        \vec{\mathbf{w}}_k^\top \cdot \vec{\mathbf{x}}^\mu &> 
        \vec{\mathbf{w}}_i^\top \cdot \vec{\mathbf{x}}^\mu \\
...
% ======================================= %
\subsection{Local optima and optimal number of clusters} \label{sbs:number_clusters}
...
\bigskip
Another crux of K-Means clustering is that the optimal number of clusters is unknown. There 854,064 players in the on-line game data-set, and it has no predefined information of how many groups showing what the similar learning patterns are. To estimate optimal number of clusters, Elbow method with Bayes and Akaike Information Criteria (\ref{eq:AICandBIC}) was used. The criteria which can determine the number of cluster is as follow,
\
...
\bigskip
Fig.~\ref{fig:AIC_BIC_Elbow} shows the information of elbow method with AIC and BIC to define the number of clusters for the on-line game data-set. By these three graphs, it seems that the data-set can be clustered into n clusters. %Difference between 13K and 14K is xx and, 14 K and 15 K is yy. 
If the number of 14 is set as a threshold for these differences, increasing K beyond 14 would not improve the result of clustering.
...
For example of test data set for 7 clusters in (\ref{fig:dead_state}), neuron 6 was never fired and it is the dead unit, even though neuron 1 and 2 won over 1600 times.
...
\begin{equation}
\begin{split}
\textrm{first order model}:&\quad f(x_n;\; \textbf{\textrm{w}}) = \textbf{\textrm{w}}^\top \textbf{\textrm{x_n}}\\
...
\begin{split}
    \textrm{vector}:&\quad f(x_n;\; \textbf{\textrm{w}}) = \textbf{\textrm{w}}^\top \textbf{\textrm{x}}_n,\\
    \textrm{matrix}:&\quad f(\textbf{\textrm{x}};\; \textbf{\textrm{w}}) = \textbf{\textrm{X}} \textbf{\textrm{w}},
\end{split}
\end{equation}
where $\textbf{\textrm{w}}$ is a vector containing parameters, and $\mathbf{X}$ is a matrix consisted with data vector. For instance, a second order polynomial model has a data vector as $\textbf{\textrm{x}}_n=[1,\; x_n,\; x_n^2]^\top$, and in matrix form,
\[
\begin{split}
    \mathbf{X} = 
        \left[ \begin{array}{c}
...
\begin{figure}[htpb]
    ...
    \framebox{\parbox[t][0.7\textwidth][t]{0.95\textwidth}{
        \begin{center}
            \includegraphics[width=0.95\textwidth, height=0.7\textwidth]{Functions.png}
        \end{center} }}
	\caption{Possible 4 models; first order polynomial, 3 parameters exponential function, 4 parameters power function, step-like function with change point at 5}
	\label{fig:Functions}
\end{figure}
...
Although high order polynomial functions are non-linear on function itself, they are still linear in parameters. In polynomial function, the increasing number of order, the lower cost they return. This situation is known as 'over fitting' problem in machine learning. Over fitting problem is common and must be solved in function fitting. This will be discussed in session \ref{sbs:over-fitting}, overfitting.
...
% ======================================= %
\section{Method}
It was discussed throughout the previous chapter what kind of models might be contained in the on-line data set as the individual learning curves. However, the on-line data set seems to be containing many noises, and they make it hard to identify true parameters of the models with such high disturbance. Furthermore, even after feasible parameters are found, criteria which measure how the trained learning curves fit well on the predefined models are necessary. If not, there is no way to distinguish the best individual learning curve among various outcomes.

Therefore, some appropriate methods have to be operated through the curve fitting process. This session introduces the curve fitting method and relevant criteria.

\subsection{Squared Lost Function} \label{sbs:square_lost_function}
There are several optimality criteria to search for optimal parameters. Squared lost function is one of the possible and well known methods to fit the linear model \cite{james2013introduction} which is linear itself or in parameters. The best learning curve may consist of the parameters producing a line that passes as many and as close as possible to the data points.

\bigskip
The squared lost function is defined as below,
\begin{equation} \label{eq:loss_function}
\begin{split}
    \mathcal{L}_{n}(y_n,\; f(x_n; \textbf{\textrm{w}})) =& (y_n - f(x_n;\; \textbf{\textrm{w}}) )^2,\\
    \mathcal{L}_{\textrm{avg}}(\textbf{\textrm{y}}, \; f(\textbf{\textrm{x}}, \textbf{\textrm{w}}) =& \frac{1}{N}\sum_{n=1}^{N}\mathcal{L}_{n}(y_n,\; f(x_n; \textbf{\textrm{w}}))
\end{split}
\end{equation}
where $\mathcal{L}_n$ represents cost value of the difference between true value $y_n$ and trained value $f(x_n)$, and $\mathcal{L}_\textrm{avg}$ is average cost of errors loss. If the value of cost is smaller and smaller, the model with trained parameter becomes closer to the true variables at the input position $n$. In other words, the average errors loss will decrease when the trained function increases with accurate parameters to the true values. 
%The best curve will have a minimum cost of average loss discovered by tuning parameters across the entire data set as follow,

\bigskip
The $\arg\min$ of the average loss is involved in finding such geometrical turning points with respect to parameters, and it will tell the minima corresponding to the turning points,
\begin{equation} \label{eq:avg_loss_function}
\begin{split}
    \textbf{w}_{\textrm{opt}} = {\arg\min}_{\textbf{w}} \frac{1}{N}\sum_{n=1}^{N}(y_n - f(x_n;\; \textbf{w}) )^2. %\textbf{\textrm{w}})) =  
\end{split}
\end{equation}

\bigskip
One way to identify those turning points with respect to parameters is using first and second partial derivatives, using vector and matrix formation. Substituting (\ref{eq:vector_matrix}) to the loss function will lead to (\ref{eq:loss_function}),
\begin{equation} \label{eq:loss_derivative}
\begin{split}
    \frac{\partial \mathcal{L}_{\textrm{avg}}}{\partial \textbf{w}} &=
    \frac{\partial}{\partial \textbf{w}}
    \bigg[ \frac{1}{N}\sum_{n=1}^{N}\mathcal{L}_{n}(y_n - \textbf{w}^\top\textbf{x}_n)^2 \bigg] \\
    &= \frac{\partial}{\partial \textbf{w}}
    \bigg[\frac{1}{N}(\textbf{y} - \mathbf{X}\textbf{w})^\top(\textbf{y} - \mathbf{X}\textbf{w}) \bigg]\\
    &= \frac{\partial}{\partial \textbf{w}}
    \bigg[\frac{1}{N}\textbf{w}^\top\mathbf{X}^\top\mathbf{X}\textbf{w} - 
    \frac{2}{N}\textbf{w}^\top\mathbf{X}^\top\textbf{y} + 
    \frac{1}{N}\textbf{y}^\top\frac{1}{N}\textbf{y}
    \bigg]
\end{split}
\end{equation}

First derivatives of the model (or gradient of the function) must be zero with respect to the turning points in a graph in parameters. Thus (\ref{eq:loss_derivative}) results in the solution,
\begin{equation} \label{eq:loss_zero}
\begin{split}
    \frac{\partial \mathcal{L}_{\textrm{avg}}}{\partial \textbf{w}} =
    \frac{2}{N}\mathbf{X}^\top\mathbf{X}\textbf{w} &-\frac{2}{N}\mathbf{X}^\top\textbf{y} = 0\\
    \mathbf{X}^\top\mathbf{X}\textbf{w} &= \mathbf{X}^\top \textbf{y}
\end{split}
\end{equation}
Then, (\ref{eq:loss_zero}) is solved with respect to $\textbf{w}$, using the Python command \textbf{\textit{numpy.linalg.solve}} or QR decomposition,
\begin{equation}
\begin{split}
    \mathbf{X}^\top \mathbf{X} \textbf{w} & = \mathbf{X}^\top \mathbf{y} \\
    (\mathbf{Q}\mathbf{R})^\top (\mathbf{Q}\mathbf{R})\textbf{w} &=
    (\mathbf{Q}\mathbf{R})^\top \mathbf{y} \\
    \mathbf{R}^\top (\mathbf{Q}^\top \mathbf{Q}) \mathbf{R} \textbf{w} &=
    \mathbf{R}^\top \mathbf{Q}^\top \mathbf{y} \\
    \mathbf{R}^\top \mathbf{R} \textbf{w} &= \mathbf{R}^\top \mathbf{Q}^\top \mathbf{y} \\
    \mathbf{R} \textbf{w} &= \mathbf{Q}^\top \mathbf{y},
\end{split}
\end{equation}
where $\mathbf{Q}$ is orthogonal matrix, so that $\mathbf{Q}^\top \mathbf{Q}=\mathbf{I}$, and matrix $\mathbf{R}$ is upper triangular. QR decomposition may provide a solution with more numerical stability, because $\mathbf{X}\top \mathbf{X}$ is removed, in which all elements are squared and then potentially numerical precision disappears. However, QR decomposition will operate on X directly, preserving the model's numerical precision.
Finally, second derivatives show that whether the turning  points correspond to minimum, maximum or saddle point. 
%Using the vector and matrix formation for linear functions in parameters, the partial derivative develops simple task.
%A potential minimum can be derived easily; computing the partial derivatives, equating to zero, and solving for parameters.

\bigskip
For non-linear function, it is not tractable to find the optimal parameters by equating partial derivatives to zero, and trans-positioning matrices in order to remain parameter matrix. In addition, to set the parameter and variable matrices is not impossible in non-linear functions with respect to parameters. In this case, 'gradient descent' method is used, which minimises the squared lost function.

\bigskip
\begin{algorithmic}
\STATE Initialise $p_n$ \COMMENT{there are $n$ number of parameters}
\STATE Set $\eta$ \COMMENT{learning rate prevents overshoot problem}
\WHILE{$p_{n\_new}-p_{n\_old}$ is not greatly smaller}
     \FOR{$i=0$ to $n$}
         \STATE $p_{n\_new} \leftarrow p_{n\_old} + \eta \frac{\partial \mathcal{L}}{\partial p_n}$
     \ENDFOR
\ENDWHILE
\end{algorithmic}

\bigskip
Through these two methods, it can be determined that the optimal parameters results in the best learning curve. For convenience of these methods, Python command scipy.optimize.least\_squares is utilised, which "solve a nonlinear least-squares problem with bounds on the variables" \cite{pythonLeastSquare}, and returns costs and other useful arguments.

\bigskip
Squared lost function is used not only to find the optimal parameters, but also for the criteria to compare the fitted learning curves. For this analysis, it is assumed that the best fitting learning curve among different models will have a minimum cost.

% ======================================= %
%		3.2.3 Training Set and Validation Set
% ======================================= %
\subsection{Training Set and Validation Set} \label{sbs:Training_Validation}
Separating the entire data set into two parts; training set and validation set, is the method to deal with the over-fitting problem. Ideally, the best fitting learning curve can generalise predictions close to the true value \cite{rogers2015first}. Therefore, separating the individual data-set with 300 attempts to two parts would have the same role of the generalisation. Although the model fits well on the training set and has very small cost, the generalised predictions from the model for an unseen data may have great disparity with big errors loss.

Therefore, to choose the best function among a set of models, each model is trained on the reduced training set, and then the corresponding costs are computed on the validation set. 

\bigskip
Particularly, training and validation set is used for identifying the best fitting single curve to the data set among four functions; polynomial, exponential, power and step-like functions. 

At first, assumption for learning processes will be tested on early in learning of 14 data set, in which it is assumed that individuals with similar performances early in learning may show similar learning patterns after 15 attempts. 
Next, the entire 300 attempts data set is reduced to the data set containing from 1 to 50 practice, which is supposed that the players practised enough.
...
\begin{subfigure}{0.5\textwidth}
    ...
    \framebox{\parbox{0.95\textwidth}{
        \begin{center}
            \includegraphics[width=0.9\textwidth, height=0.6\textwidth]{exp3_15sample_1.png}
        \end{center} }}
    \caption{Training set}
    \label{fig:train_valid_1_a}
\end{subfigure}
\begin{subfigure}{0.5\textwidth}
    \framebox{\parbox{0.95\textwidth}{
        \begin{center}
            \includegraphics[width=0.9\textwidth, height=0.6\textwidth]{exp3_15sample_2.png}
        \end{center} }}
    \caption{Validation set}
    \label{fig:train_valid_1_b}
\end{subfigure}
\caption{Minimum cost in training set, but large cost in validation set. Exponential function fitting with 3 parameters to 15 training set of cluster 13, and validation check}
\label{fig:train_valid_1}
\end{figure}
...
\begin{subfigure}{0.5\textwidth}
    ...
    \framebox{\parbox{0.95\textwidth}{
        \begin{center}
            \includegraphics[width=0.9\textwidth, height=0.6\textwidth]{exp3_15valid_1.png}
        \end{center} }}
    \caption{Training set}
    \label{fig:train_valid_2_a}
\end{subfigure}
\begin{subfigure}{0.5\textwidth}
    \framebox{\parbox{0.95\textwidth}{
        \begin{center}
            \includegraphics[width=0.9\textwidth, height=0.6\textwidth]{exp3_15valid_2.png}
        \end{center} }}
    \caption{Validation set}
    \label{fig:train_valid_2_b}
\end{subfigure}
\caption{The learning curve which has the minimum cost in validation set. Exponential function fitting with 3 parameters to 15 training set of cluster 1, and validation check}
\label{fig:not-fig:train_valid_2}
\end{figure}
...
% ======================================= %
\subsection{Over-fitting and Regularisation} \label{sbs:over-fitting}
The term of over-fitting in machine learning is the case that "a model is too complex and is using its surplus complexity to fit to noise" \cite{rogers2015first}. For example, in polynomial model, the higher order the function is, the lower errors loss the model will have. It makes sense that the best learning curve will have the least cost in different single functions. However, these criteria cannot afford the comparison method between a single function and piecewise model, because separating the data range as small as possible means fitting the data set into small intervals, and will lead definitely and undoubtedly to smaller cost (over-fitting), such as the situation that increasing the number of clusters leads to smaller mean square errors in session (\ref{sbs:number_clusters}). 

\begin{figure}[h]
    \centering
    \framebox{\parbox[t][0.4\textheight][t]{0.95\textwidth}{
        \begin{center}
            \includegraphics[width=0.9\textwidth, height=0.38\textheight]{elbow_avg4_power_3param.png}
        \end{center} }}
	\caption{Regularisation example of cluster 4 in the piecewise power function with 3 parameters.}
    \label{fig:pieces}
\end{figure}
...
\begin{figure}[h]
    ...
    \framebox{\parbox[t][0.4\textheight][t]{0.95\textwidth}{
        \begin{center}
            \includegraphics[width=0.9\textwidth, height=0.38\textheight]{elbow_step_5.png}
        \end{center} }}
	\caption{Regularisation example of cluster 5 in the piecewise step-like functions.}
    \label{fig:pieces}
\end{figure}

\bigskip
Regularisation prevents the model becoming too complex, by penalising over complexity \cite{rogers2015first}. Likewise the solution of k-mean clustering with competitive learning algorithm, 'Bayes information criteria' (BIC) and 'Akaike Information Criteria' (AIC), equation (\ref{eq:AICandBIC}), are used as the regularisation criteria. 


% ======================================= %
%		3.2.4 Choice of Breaking Point
% ======================================= %
\subsection{Choice of Breaking Point} \label{abs:breaking_point}
Fig. \ref{fig:pieces} shows an example of how to find the exact transition points. Firstly, the entire data range is separated to $n-1$ transition points in $n$ pieces. Thus, there are three lines as the transition points with different colours (red, green, blue). A minimum of 50 window in each piece is used for the example, though actual window is 10. Accordingly, transition point 1 is on 50, point 2 is on 100, and the last point is on 150, and the last piece has remained 150 of attempt range as an initial condition. Fig. (\ref{fig:pieces}) shows this example of the transition point shifts.

\bigskip
In each of the pieces, a single function's cost is saved into a matrix. At the same time, length and transition points of each piece is also saved into another matrix. The shape for both matrices are $[\textit{\textrm{ number of pieces $\times$ number of shift of a last transition point}}]$. (\ref{eq:cost_matrix}) and (\ref{eq:length_matrix}) are examples of the cost matrix and length matrix for 3 transition points with 4 pieces.
...
\begin{figure}[thpb]
    \centering
    \framebox{\parbox[t][0.6\textheight][t]{0.95\textwidth}{
        \begin{center}
            \includegraphics[width=0.95\textwidth, height=0.56\textheight]{piece_test.png}
        \end{center} }}
	\caption{Transition point shift in 4 pieces}
    \label{fig:transition_shift}
\end{figure}
...
% ======================================= %
\chapter{Discussion}
The traditional power law model (single smooth power function) is successfully fitted to the entirely averaged on-line game data set than the exponential law model. It became obvious that the power law model is the artificial learning curve comprised of several practice law models. 

After the entire data set is categorised into the 13 groups through the data curation task, in which individuals show the similar learning patterns early in learning, each group provides different shape of learning curves. The most-frequent function is piece wise model, in which individuals follow the power law model with 4 parameters combined with step-like or first order polynomial models. 

\bigskip
It seems that power law model is still dominant in those 13 clusters rather than exponential or other models. The reason of that may be that those groups are still averaged data set over 1000 individuals having similar early learning patters. Because the raw data of the on-line game for almost all individuals have discontinuous data sequences; for example individual A has the data ranging from 40 to 90 practice, or the data of individual B starts attempt 1 with practice range 20 and the second interval starts 130 attempt with practice range 40, the function fitting for each individual could not be implemented, and thus players' data are clustered on the basis of the early 15 attempts, and are averaged instead.

% ======================================= %
%		4.1 The Best Learning Curve
% ======================================= %
\section{The Best Learning Curve}
As discussed above, those 13 groups have different shape of learning curves. The most of them are piece wise function, while cluster 3 and 8 have a single first order polynomial function and power function with 4 parameters, respectively. 

\bigskip
Firstly, the argument insisted by Murr and Chessa (2011) is tested in session (\ref{sba:PL_EL}), which is that the exponential learning curve with 3 parameters describes the individual learning process better than power law model, and the power law model is the representative of averaging those individuals.

Next, the result of differences between the single and multiple curves for each cluster are discussed in the following session. Those clusters are the reduced data set from the entirely averaged data set.
% ======================================= %
%		4.1.2 The Best Learning Curve
% ======================================= %
\subsection{Power Law and Exponential Law} \label{sba:PL_EL}
In single learning curves, the power law model with 4 parameters are the best fitting model to the entirely averaged on-line game data set with minimum cost, 2.94. Among exponential functions with different number of parameter, 3 parameter exponential model has the minimum cost, 3.51. 
% Fig.
\graphicspath{ {Figs/} }
\begin{figure}[htpb]
\begin{subfigure}{0.5\textwidth}
    ...
    \framebox{\parbox{0.95\textwidth}{
        \begin{center}
            \includegraphics[width=0.9\textwidth, height=0.6\textwidth]{single_exponential3.png}
        \end{center} }}
    \caption{Exponential model with 3 parameters}
    \label{fig:single_exponential3}
\end{subfigure}
\begin{subfigure}{0.5\textwidth}
    \framebox{\parbox{0.95\textwidth}{
        \begin{center}
            \includegraphics[width=0.9\textwidth, height=0.6\textwidth]{single_powerlaw4.png}
        \end{center} }}
    \caption{Power law model with 4 parameters}
    \label{fig:single_powerlaw4}
\end{subfigure}
\caption{Comparison between exponential function and power function. Both are the best fitting learning curve to the entirely averaged data set among different number of parameters.}
\label{fig:single_functions}
\end{figure}

Therefore, it is assumed that when all performances of players are averaged together, learning dynamics disappears, and the global learning curve follows the power law.
% ======================================= %
%		4.1.3 The Best Learning Curve
% ======================================= %
\subsection{Single and Piecewise Curves}
For clustered individuals, four functions; polynomial, exponential, power, and step functions, are tested through the possible all situations. As well as single curve fitting, piecewise cases in different number of pieces with window 30 are examined, by multiplying a function or combining  different functions together. After that, all squared errors loss of those situations are compared with each other, and the functions which has the least cost value are founded. Though this, the tasks for function fitting become a variety. 

\bigskip
Three multiple functions comprised by a single model are iterated 10 times in order to get the costs under the different number of pieces. These functions are as follow,
\begin{itemize}
    \item piecewise exponential model,
    \item piecewise power model,
    \item piecewise step-like model.
\end{itemize}

\bigskip
Next, combinations of those functions are fitted to the averaged cluster data sets. 
\begin{itemize}
    \item piecewise exponential and step-like model,
    \item piecewise exponential and polynomial model,
    \item piecewise power and step-like model,
    \item piecewise power and polynomial model.
\end{itemize}

\bigskip
Fig. \ref{fig:best_fit} shows the result of the function fitting. 11 clusters have piecewise functions, while two clusters; cluster 3 and cluster 8, have a continuous single curve; first order polynomial model and power model with 4 parameters. Interestingly, first order polynomial function is fitted best to the individuals in cluster 3. Their performances improves gradually, but the rapid improvement at the early phase in learning with similar to other clusters does not exist. Therefore, they can be assumed as a group who learns slowly but steadily. Although their learning process is slow, they might be expert after sufficient time, because the learning curve increase without limitation, while others approaches to the asymptotes. 

\bigskip
The most dominant model is piecewise power law. In piecewise functions, power law models always appear before the first transition point.  7 clusters have piecewise power curves, not only in the early part of learning before the first transition point, but also in the later trials. 3 groups have a power curve in early attempts, followed by the polynomial functions. Only cluster 7 have the combination of piece wise functions for power and step-like models.

\bigskip
In additions, most first transition points appear after about 200 practice when they experience the stable learning process, which means the learning curves approaches asymptotes. Thus, through these two results that power model exists in almost all piecewise learning processes, and the first transition point appears when learning speed become significantly slow, the hypothesis can be established that individuals may follow the power law to learn any kind of new skills in practice, then they attempt different learning methods in order to exceed the currently slow learning process. 

\bigskip
%fig1
\graphicspath{ {Figs/Learning_curves/} }
\begin{figure}[htpb]
\begin{subfigure}{0.5\textwidth}
    \centering
    \framebox{\parbox{0.95\textwidth}{
        \begin{center}
            \includegraphics[width=0.9\textwidth, height=0.6\textwidth]{avg1_4param.png}
        \end{center} }}
    \caption{Cluster 1: Power and polynomial functions}
\end{subfigure}
\begin{subfigure}{0.5\textwidth}
    \framebox{\parbox{0.95\textwidth}{
        \begin{center}
            \includegraphics[width=0.9\textwidth, height=0.6\textwidth]{avg2_4param.png}
        \end{center} }}
    \caption{Cluster 2: Power functions}
\end{subfigure}
\end{figure}
%fig2
\graphicspath{ {Figs/Learning_curves/} }
...
\begin{subfigure}{0.5\textwidth}
    ...
    \framebox{\parbox{0.95\textwidth}{
        \begin{center}
            \includegraphics[width=0.9\textwidth, height=0.6\textwidth]{avg3.png}
        \end{center} }}
    \caption{Cluster 3: Single polynomial function}
\end{subfigure}
\begin{subfigure}{0.5\textwidth}
    \framebox{\parbox{0.95\textwidth}{
        \begin{center}
            \includegraphics[width=0.9\textwidth, height=0.6\textwidth]{avg4_4param.png}
        \end{center} }}
    \caption{Cluster 4: Power functions}
\end{subfigure}
\end{figure}
%fig3
\graphicspath{ {Figs/Learning_curves/} }
...
\begin{subfigure}{0.5\textwidth}
    ...
    \framebox{\parbox{0.95\textwidth}{
        \begin{center}
            \includegraphics[width=0.9\textwidth, height=0.6\textwidth]{avg5_4param.png}
        \end{center} }}
    \caption{Cluster 5: Power and polynomial functions}
\end{subfigure}
\begin{subfigure}{0.5\textwidth}
    \framebox{\parbox{0.95\textwidth}{
        \begin{center}
            \includegraphics[width=0.9\textwidth, height=0.6\textwidth]{avg6_4param.png}
        \end{center} }}
    \caption{Cluster 6: Power functions}
\end{subfigure}
\end{figure}
%fig4
\graphicspath{ {Figs/Learning_curves/} }
...
\begin{subfigure}{0.5\textwidth}
    ...
    \framebox{\parbox{0.95\textwidth}{
        \begin{center}
            \includegraphics[width=0.9\textwidth, height=0.6\textwidth]{avg7_4param.png}
        \end{center} }}
    \caption{Cluster 7: Power and step-like functions}
\end{subfigure}
\begin{subfigure}{0.5\textwidth}
    \framebox{\parbox{0.95\textwidth}{
        \begin{center}
            \includegraphics[width=0.9\textwidth, height=0.6\textwidth]{avg8_power_single.png}
        \end{center} }}
    \caption{Cluster 8: Single power function}
\end{subfigure}
\end{figure}
%fig5
\graphicspath{ {Figs/Learning_curves/} }
...
\begin{subfigure}{0.5\textwidth}
    ...
    \framebox{\parbox{0.95\textwidth}{
        \begin{center}
            \includegraphics[width=0.9\textwidth, height=0.6\textwidth]{avg9_4param.png}
        \end{center} }}
    \caption{Cluster 9: Power functions}
\end{subfigure}
\begin{subfigure}{0.5\textwidth}
    \framebox{\parbox{0.95\textwidth}{
        \begin{center}
            \includegraphics[width=0.9\textwidth, height=0.6\textwidth]{avg10_4param.png}
        \end{center} }}
    \caption{Cluster 10: Power functions}
    \label{fig:best_fit_5_b}
\end{subfigure}
\end{figure}
%fig6
\graphicspath{ {Figs/Learning_curves/} }
...
\begin{subfigure}{0.5\textwidth}
    ...
    \framebox{\parbox{0.95\textwidth}{
        \begin{center}
            \includegraphics[width=0.9\textwidth, height=0.6\textwidth]{avg11_4param.png}
        \end{center} }}
    \caption{Cluster 7: Power and polynomial functions}
\end{subfigure}
\begin{subfigure}{0.5\textwidth}
    \framebox{\parbox{0.95\textwidth}{
        \begin{center}
            \includegraphics[width=0.9\textwidth, height=0.6\textwidth]{avg12_4param.png}
        \end{center} }}
    \caption{Cluster 7: Power functions}
    \label{fig:best_fit_6_b}
\end{subfigure}
\end{figure}
%fig7
\graphicspath{ {Figs/Learning_curves/} }
...
\begin{subfigure}{0.5\textwidth}
    ...
    \framebox{\parbox{0.95\textwidth}{
        \begin{center}
            \includegraphics[width=0.9\textwidth, height=0.6\textwidth]{avg13_4param.png}
        \end{center} }}
    \caption{Cluster 7: Power functions}
\end{subfigure}
%\begin{subfigure}{0.5\textwidth}
...
%        \end{center} }}
%    \caption{Validation set}
%    \label{fig:best_fit_7_b}
%\end{subfigure}
%\caption{Minimum cost in training set, but large cost in validation set. Exponential function fitting with 3 parameters to 15 training set of cluster 13, and validation check}
\caption{The result of the best function fitting curves for each clusters.}
\label{fig:best_fit}
\end{figure}

\newpage
\section{Exploitation and Exploration}
The process of the skill acquisition in practice can be regarded as the theoretical framework of reinforcement learning \cite{stafford2014tracing}. The framework of reinforcement leaning is based on the situation in which the current decision is taken after considering the future rewards and states. A policy in the reinforcement learning affects the way of decisions at a given time . In other world, "a policy is a mapping from perceived states of the environment to actions to be taken when in those states" \cite{sutton1998reinforcement}. Depends on the policy, agents may take an action greedily, exploiting, or non-greedily, exploring. The exploration in reinforcement learning represent the case that agents take an action under the currently given or known value of the actions. However, there would be the better results beyond the current knowledge and experiences. In this case, the exploration can provide possibility of the improvement in the estimate of expected rewards, while the outcome after taking the action for the exploration would result in not superior or even worse rewards. Thus, agents are controlled by the policy of which action between exploitation and exploration will take, in order to maximise the future result. 

\bigskip
The same as the problem of the trade-off in reinforcement learning, individuals who play the on-line game decide an action whether they will exploit the current knowledge (otherwise, follow the instruction of the game) or explore the game under several plays, in order get the higher scores. Of course, the on-line game play is involved in not only "decision making", but also "motor responding", and "rapid perception" \cite{stafford2014tracing}. However, the results of the function fitting to the on-line game data set may imply the framework how players solve the trade-off.

\bigskip
The practice before the first transition point for almost all clusters shows the monotonic improvement in the early phases, getting slower and slower. Finally it approaches the asymptote. All of them, except cluster 3 which has polynomial learning curve, follow this power law learning process. At the end of the power curve, which is near to asymptote, scores become fluctuated until it reaches to the first transition, then they show different shape of learning curves with significantly fluctuated scores; but higher than the value of asymptotes.

\bigskip
Therefore, individuals tend to take the exploitation actions at the beginning of the game, and they does not choose the exploration actions at the beginning. Then, when they feel that their learning process became significantly slow, they try and explore different methods, in order to improve their performance.
%\section{A Power Law : An Artificial Curve}
...
Gantt charts are attached as bellow.
\end{document}